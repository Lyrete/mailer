...
foreach ($wholeresult as $row){
    if(($row["startDate"] >= date('Y-m-d') and strtotime($row["startDate"]) < strtotime($deadline->format('Y-m-d'))) or ($row["endDate"] != NULL and $row["endDate"] >= date("Y-m-d") and $row["startDate"] < date("Y-m-d"))){
